...
const COLUMNS = [
    ...
    Cell: ({ row }: any) => {
      ...
      return (
        <div className="flex items-start gap-1">
          <h3
            title={
              row?.original?.txHash > 9 ? row?.original?.txHash : undefined
            }
  ...
  },
  {
    ...
    Cell: ({ row }: any) => (
      <div className="flex items-start gap-2">
        <h3
          title={
            row?.original?.childDAO?.name?.length > 9
              ? row?.original?.childDAO?.name
              : undefined
          }
          className="text-left"
        >
          {row?.original?.childDAO?.name?.length > 15
            ? `${row?.original?.childDAO?.name?.substring(0, 10)}...`
            : row?.original?.childDAO?.name}
        </h3>
      </div>
    ),
  ...
  },
  {
    ...
    Cell: ({ row }: any) => {
      return (
        <a
          href={`https://sepolia.basescan.org/tx/${row?.original?.txHash}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex cursor-pointer items-center justify-end gap-2 text-[#1D4ED8]"
        >
...
export default function RewardDistributionTable() {
  ...
  return (
    <RewardAccordionTable columns={columns} data={data} isLoading={isLoading} />
  );
}
